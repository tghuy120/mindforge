...
### CLAUDE.md vs Memory：Framing 效应——加载了为什么还是不管用？

上面的对比表解释了加载**策略**的差异，但在实践中还有一个更隐蔽的问题：**即使 Memory 已经加载到 Context 中，它的行为约束力也远弱于 CLAUDE.md。** 原因不在于加载机制，而在于系统级的 **framing（框架效应）**。

#### 系统指令对两者的定位

Claude Code 在注入这两种内容时，使用了完全不同的系统指令：

| 来源 | 系统指令 framing | 隐含语义 |
|------|----------------|---------|
| CLAUDE.md | "project instructions... **MUST follow** exactly as written, **OVERRIDE** any default behavior" | 命令式：必须执行 |
| Memory | "user's auto-memory... this context **may or may not be relevant**... should **not respond** unless highly relevant" | 参考式：酌情采纳 |

模型会根据这些 framing 自动设定优先级——"MUST follow" 的内容权重远高于 "may or may not be relevant" 的内容。**这意味着，当 Memory 中的规则与模型的默认行为模式冲突时，默认行为很可能胜出。**

#### 真实案例：URL 访问路由

一个亲身经历的失败案例：

- **Memory 中的规则**："认证网页（chatgpt.com、docs.google.com）必须用 Playwright 访问，不要先试 WebFetch"
- **模型的默认行为**：遇到 URL 时，先用轻量工具（tavily/WebFetch），失败后再用重工具（Playwright）
- **实际结果**：Memory 规则被完全忽略，模型按默认策略先试 tavily → 再试 WebFetch → 最后才用 Playwright，浪费了两次无效尝试

根因：**Memory 的 framing 告诉模型"这些信息可能相关也可能不相关"，模型在决策时没有把它当做必须遵守的规则。**

#### 行为规则 vs 背景知识——Memory 中的两类信息

这个案例揭示了一个关键区分：Memory 中存储的信息并非同质的。

| 类型 | 示例 | 需要的约束力 | Memory 能提供的 |
|------|------|------------|---------------|
| **行为规则**（"遇到 X 必须做 Y"） | "认证 URL 用 Playwright" | 强——必须执行 | 弱——"可能相关" |
| **背景知识**（"X 是什么"） | "用户是 AI 工程师" | 弱——了解即可 | 足够——提供上下文 |

**结论**：Memory 的 framing 适合存储背景知识，但**不适合存储行为规则**。行为规则（"做 X 必须用 Y"、"禁止做 Z"）应该放在 CLAUDE.md 中，因为只有 CLAUDE.md 的 framing 能提供足够的约束力。

#### Memory Promotion Protocol（记忆提权协议）

基于以上分析，建议建立"提权"机制——当 Memory 中的反馈被识别为行为规则时，主动提权到 CLAUDE.md：

```
Memory 反馈分类：
├── 背景知识（用户画像、项目上下文）→ 留在 Memory ✅
└── 行为规则（"必须做X"、"禁止做Y"）→ 提权到 CLAUDE.md ⬆️
    ├── 一般行为规则 → CLAUDE.md Key Rules section
    └── 绝对禁止类规则 → 评估是否需要 Hook（L1 确定性约束）
```

**提权触发条件**：
- 用户的反馈包含"必须"、"不要"、"禁止"等指令性词汇
- Memory 中的行为规则被违反过一次
- 用户明确说"绝对不要"、"永远禁止"

**注意**：这不意味着 Memory 无用。Memory 对于背景知识、项目上下文、资源指针等"软信息"仍然是最合适的存储位置——这些信息不需要"强制执行"，只需要"被了解"。提权的目标是**把错配的信息放到正确的位置**，而不是废弃 Memory。
...
### 6. 区分 Memory 中的"行为规则"和"背景知识"

Memory 的系统 framing（"may or may not be relevant"）适合存储背景知识，但不足以约束行为规则。定期审查 Memory 中的 feedback 类条目：

- **包含"必须"、"禁止"、"不要先试"等指令性词汇** → 提权到 CLAUDE.md
- **描述用户身份、项目状态、资源位置** → 留在 Memory

参考"加载机制深度解析"中的 **Memory Promotion Protocol** 部分了解完整的提权机制。
